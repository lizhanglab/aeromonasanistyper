...
def ksi(fastANI_output, args):
    ...
    for row in reader:
        ani_species[row["genome"]] = row["species"]
    ...
    with open(args.output, "w") as output_file:
        output_file.write("Query Genome\tHighest ANI similarity\tReference Genome with the highest ANI similarity\tReference ANI Species\n")
        
        for query_genome, (highest_ani, reference_genome) in highest_ani_values.items():
            reference_species = ani_species.get(reference_genome, "UNKNOWN_REFERENCE")
            
            if round(highest_ani, 1) >= 95.2:
                output_file.write(f"{query_genome}\t{highest_ani}\t{reference_genome}\t{reference_species}\n")
            else:
                output_file.write(
                    f"{query_genome}\t{highest_ani}\t{reference_genome}\tNovel (closest {reference_species})\tPotential Novel Aeromonas ANI species\n")
